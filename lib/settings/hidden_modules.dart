          ...
          children: [
            SizedBox(
              ...
              child: OutlinedButton.icon(
                onPressed: () => setState(() => _hiddenModulesExpanded = !_hiddenModulesExpanded),
                icon: Icon(
                  _hiddenModulesExpanded ? Icons.expand_less : Icons.expand_more,
                  color: kAccentLight,
                  size: 30,
                ),
                label: Text("Hidden Modules", style: TextStyle(color: kTextPrimary, fontSize: 15)),
            ...
            AnimatedSize(
              ...
              child: _hiddenModulesExpanded
                ...
                child: DropdownButtonFormField<double>(
                  ...
                  items: widget.visibleTools.map((t) => DropdownMenuItem<double>(
                      value: t.id,
                      child: Row(
                          children: [
                            Icon(Icons.visibility, size: 20, color: kTextSecondary),
                            SizedBox(width: 5),
                  ...
                  onChanged: (id) {
                    if (id == null) return;
                    widget.onHide(id);
                    setState(() {_hideDropdownKey.currentState?.reset();});
                  },
                ),
              )
              ...
              Expanded(
                  child: Align(
                    ...
                    child: Padding(
                      ...
                      child: SizedBox(child: Icon(Icons.subdirectory_arrow_right, size: 30, color: kAccentLight)),
                    ),
                  )
              ),
              Padding(
                ...
                child: Container(
                    decoration: BoxDecoration(
                      border: Border.all(color: kAccent.withAlpha(100), width: 1.5),
                      borderRadius: BorderRadius.circular(8),
                    ),
                    constraints: const BoxConstraints(minHeight: 35, maxHeight: 165, minWidth: 200),
                    child: widget.hiddenTools.isEmpty
                        ? Text("No hidden modules", style: TextStyle(color: kTextSecondary, fontSize: 13), textAlign: TextAlign.center)
                        : SingleChildScrollView(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: widget.hiddenTools.map((t) => Padding(
                            ...
                            child: GestureDetector(
                              child: Column(
                                children: [
                                  Row(
                                    children: [
                                      Text("", style: TextStyle(color: kTextPrimary, fontSize: 14)),
                                      Icon(Icons.visibility_off_outlined, size: 20, color: kTextSecondary,),
                                      Text(" ${t.name}", style: TextStyle(color: kTextPrimary, fontSize: 14)),
                                    ],
                                  ),
                                ],
                              ),
