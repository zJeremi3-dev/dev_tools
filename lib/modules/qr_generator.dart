...
class _QRGenDialogState extends State<QRGenDialog> {
  ...
  String _text = ""; int _selectedSize = 4; int _selectedQuality = QrErrorCorrectLevel.M;
  QrImage? _qrImage; String? _error;
  ...
  int? _minVersion; int _effectiveSize = 4;
  ...
  bool _fits(int typeNumber, int errorLevel, String text) {
    try {final qrCode = QrCode(typeNumber, errorLevel)..addData(text); QrImage(qrCode).moduleCount; return true;}
    catch (_) {return false;}
  }
  int? _minVersionFor(int errorLevel, String text) {
    if (text.isEmpty) return null;
    for (int v = 1; v <= 40; v++) {if (_fits(v, errorLevel, text)) return v;}
    return null;
  }
  void _generator() {
    ...
    if (minVersion == null) {
      setState(() {
        _qrImage = null;
        _error = 'Text is too long – doesn\'t fit at any size (max. version 40).';
        _minVersion = null;
      });
      return;
    }

    final effectiveSize = _selectedSize < minVersion ? minVersion : _selectedSize;
  ...
  Future<void> _downloadQr() async {
    ...
    final path = await FilePicker.platform.saveFile(fileName: 'qrcode.png', bytes: kIsWeb ? bytes : null);
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Text('QR-Code Generator', style: TextStyle(color: kTextPrimary, fontSize: 18, fontWeight: FontWeight.w700),),
              IconButton(
          ...
          buildSection(
            ...
            children: [
              TextField(
                ...
                onChanged: (value) {setState(() => _text = value); _generator();},
              ),
              const SizedBox(height: 6),
              Text('Enter the text to encode in the QR code.', style: TextStyle(color: kTextSecondary, fontSize: 11)),
            ],
          ),
              ...
              Expanded(
                child: AspectRatio(
                  ...
                  child: Container(
                      ...
                      child: _qrImage == null
                          ? Text(_error ?? 'Enter text …', textAlign: TextAlign.center, style: TextStyle(color: kTextSecondary, fontSize: 12))
                          : RepaintBoundary(
                        key: _qrKey,
                        child: SizedBox.expand(child: CustomPaint(painter: _QrPainter(_qrImage!))),
                      )
                  ),
                ),
              ),
              ...
              Expanded(
                child: buildSection(
                    ...
                    children: [
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                if (locked) ...[
                                  ...
                                  Icon(Icons.lock, size: 12, color: kTextSecondary.withAlpha(150)),
                                ],
                              ],
                            ),
                          );
                      ...
                      if (_minVersion != null && _selectedSize < _minVersion!) ...[
                        const SizedBox(height: 6),
                        Text(
                          _minVersion! > 10
                      ...
                      DropdownButtonFormField<int>(
                        ...
                        items: _qualityLabels.entries.map((e) {
                          final locked = _text.isNotEmpty && !_fits(_effectiveSize, e.key, _text);
                          return DropdownMenuItem<int>(
                            value: e.key,
                            enabled: !locked,
                            child: Row(
...
class _QrPainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(covariant _QrPainter oldDelegate) => oldDelegate.qrImage != qrImage;
}