  ...
  void convert() {
    ...
    try {
      _mode == 0
          ...
          : setState(() => _output = BigInt.parse(_input!, radix: 2).toString());
    } catch (_) {
      setState(() => _output = null);
    }
  }
                ...
                children: [
                  Expanded(
                      child: SingleChildScrollView(
                        ...
                        child: Text(
                          _mode == 0 ? "Binary: ${_output ?? ''}" : "Decimal: ${_output ?? ''}",
                          style: TextStyle(color: kTextPrimary, fontSize: 15, letterSpacing: 2, fontFamily: 'monospace'),
                        ),
                      )
                  ),
