...
BigInt _generatePrime() {
  ...
  while (true) {
    ...
    for (int i = 0; i < numBytes; i++) {bytes[i] = random.nextInt(256);}
    bytes[0] |= 0x80;
    bytes[numBytes - 1] |= 0x01;

    BigInt candidate = BigInt.zero;
    for (int byte in bytes) {candidate = (candidate << 8) | BigInt.from(byte);}
    ...
    for (int i = 0; i < 40; i++) {
      ...
      do {
        ...
        for (int j = 0; j < numBytes; j++) {aBytes[j] = random.nextInt(256);}
        BigInt temp = BigInt.zero;
        for (int byte in aBytes) {temp = (temp << 8) | BigInt.from(byte);}
      ...
      if (composite) {isPrime = false;break;}
    }
...
class _RSADialogState extends State<RSADialog> {
  ...
  String? _error;
  String? _encodedText; String? _decodedText;
  ...
  Future<void> generator() async {
    try {
      final results = await Future.wait([Isolate.run(_generatePrime), Isolate.run(_generatePrime)]);
      BigInt p = results[0]; BigInt q = results[1];
      while (p == q) { q = await Isolate.run(_generatePrime); }

      BigInt n = p * q;
      BigInt phi = (p - BigInt.one) * (q - BigInt.one);

      BigInt e = BigInt.from(65537);
      while (e.gcd(phi) != BigInt.one) {e += BigInt.two;}
  ...
  void _recalculate() {
    ...
    try {tester(1);tester(2);}
    catch (e) {setState(() {_error = e.toString().replaceAll("FormatException: ", "");});}
  }
  void tester(int art) {
    //art: 1 = encrypting, 2 = decrypting
    if (art == 1) {
      if (_inputController.text.isEmpty) {setState(() => _encodedText = null); return;}
      if (_publicController.text.isEmpty) {setState(() => _encodedText = null); return;}

      final cipherList = _encrypt(_inputController.text, _publicController.text);
      setState(() {_encodedText = cipherList.join(' ');});
    }
    if (art == 2) {
      if (_inputController.text.isEmpty) {setState(() => _decodedText = null); return;}
      if (_publicController.text.isEmpty) {setState(() => _decodedText = null); return;}
      if (_privateController.text.isEmpty) {setState(() => _decodedText = null); return;}

      final cipherText = _encodedText!.trim().split(RegExp(r'\s+')).map((e) => BigInt.parse(e)).toList();

      // Encrypting
      final resultText = _decrypt(cipherText, _privateController.text);
      setState(() {_decodedText = resultText;});
    }
  }
  List<BigInt> _encrypt(String text, String keyString) {
    ...
    return bytes.map((byte) {
      ...
      if (m >= N) {
        throw FormatException("Modulus N is too small for the character '$byte'.");
      }
      return m.modPow(e, N);
    }).toList();
  }
  ...
  static List<BigInt> _parseKey(String keyString) {
    ...
    if (parts.length != 2) {throw const FormatException("Key must be in the format 'e, n' or 'd, n'.");}
    final exp = BigInt.tryParse(parts[0]);
    final mod = BigInt.tryParse(parts[1]);

    if (exp == null || mod == null) {throw const FormatException("Invalid numbers in key.");}
    return [exp, mod];
  }
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Text('RSA Key-Pair', style: TextStyle(color: kTextPrimary, fontSize: 18, fontWeight: FontWeight.w700)),
              IconButton(
            ...
            child: Column(
              children: [
                Container(
                  ...
                  child: IconButton(
                    onPressed: (){generator();},
                    icon: privateKey == null ? Icon(Icons.play_arrow) : Icon(Icons.refresh_rounded),
                    iconSize: 22,
                    color: kAccentLight,
                  ),
                ),            // Generate Button
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      buildCopyButton(context: context, copyText: publicKey ?? "")
                    ],
                  ),
                ),            // Public Key
                const SizedBox(height: 14),
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      const SizedBox(width: 12),
                      buildCopyButton(context: context, copyText: privateKey ?? "")
                    ],
                  ),
                ),            // Private Key
              ],
                ...
                buildSection(
                  ...
                  children: [
                    TextField(
                      ...
                      autofocus: true, maxLines: 4, minLines: 1,
                      decoration: fieldDecoration('Text'),
                      style: TextStyle(color: kTextPrimary, fontSize: 14),
                      onChanged: (value) => _recalculate(),
                    ),            // Text
                    Text('Enter the text to be encrypted', style: TextStyle(color: kTextSecondary, fontSize: 11)),
                    ...
                    Text('Enter your public key for encryption', style: TextStyle(color: kTextSecondary, fontSize: 11)),
                    const SizedBox(height: 6),
                    Container(
