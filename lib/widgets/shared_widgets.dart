...
Widget buildSection({required String label, required List<Widget> children}) {
  return Container(
    ...
    child: Column(
      ...
      children: [
        if (label != "") Text(label, style: TextStyle(color: kTextSecondary, fontSize: 11, letterSpacing: 0.8)),
...
Widget buildCheckboxOption(String label, bool value, VoidCallback onTap) {
  return Row(
    ...
    children: [
      SizedBox(width: 20, height: 20, child: Checkbox(value: value, onChanged: (_) => onTap())),
...
Widget buildCopyButton({required BuildContext context, required String copyText}) {
  return Tooltip(
    message: "Copy",
    decoration: BoxDecoration(
        color: kSurfaceColor,
    ...
    child: IconButton(
      onPressed: copyText.isEmpty ? null : () {Clipboard.setData(ClipboardData(text: copyText));showCopiedSnackbar(context);},
...
class _CopiedSnackbarContentState extends State<_CopiedSnackbarContent> with SingleTickerProviderStateMixin {
