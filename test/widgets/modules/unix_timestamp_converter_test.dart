...
void main() {
  Widget wrap() => const MaterialApp(home: Scaffold(body: UnixTimestampDialog()));
  ...
  testWidgets('seconds vs. milliseconds toggle changes the interpreted date', (tester) async {
    await tester.pumpWidget(wrap());
  ...
  testWidgets('non-numeric timestamp shows an error instead of crashing', (tester) async {
    await tester.pumpWidget(wrap());
  ...
  testWidgets('date-to-timestamp mode produces a timestamp for the selected date', (tester) async {
    await tester.pumpWidget(wrap());
    ...
    final text = tester.widget<Text>(find.textContaining('Timestamp: ')).data!;
